...
interface NewFrontmatter {
  ...
  [key: string]: any; // Allow other legacy fields during transition
}
 ...
 */
function cleanAIMetadata(aiMetadata: any): NewFrontmatter['ai_metadata'] | undefined {
  if (!aiMetadata) return undefined;
  
  // Extract translation metadata if it exists
  const tokenUsage = aiMetadata.tokenUsage?.translation;
  if (tokenUsage) {
    return {
      translation: {
        model: tokenUsage.model,
        at: tokenUsage.timestamp || new Date().toISOString(),
        sourceLanguage: tokenUsage.sourceLanguage || 'en',
        targetLanguage: tokenUsage.targetLanguage || 'de',
        tokens: tokenUsage.totalTokens || tokenUsage.tokens,
        cost: tokenUsage.cost,
        co2: tokenUsage.co2Impact || tokenUsage.co2,
      }
    };
  }
...
function migrateFrontmatter(frontmatter: LegacyFrontmatter): NewFrontmatter {
  ...
  const migrated: NewFrontmatter = {
    ...
    firstPublishedAt: frontmatter.firstPublishDate || frontmatter.firstPublishedAt,
    updatedAt: frontmatter.lastChangeDate || frontmatter.updatedAt,
  };
 ...
 */
async function migrateFrontmatterFiles(dryRun: boolean = true) {
  ...
  if (dryRun) {
    ...
    console.log(`   node scripts/migrate-frontmatter.mjs --live`);
  } else {
