...
import { ContentAnalyzer, type ContentAnalysis } from './content-analyzer.js';
...
async function analyzeContentForTags(): Promise<TagSuggestionReport> {
  ...
  const suggestionsOnly = analyses.filter(analysis => 
    analysis.suggestedTags.length > 0 || analysis.newTagCandidates.length > 0
  );
  ...
  const totalSuggestedTags = analyses.reduce((sum, analysis) => 
    sum + analysis.suggestedTags.length, 0
  );
