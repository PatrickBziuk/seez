...
class ContentAnalyzer {
   ...
   */
  private analyzeContentSemantics(content: string, title: string, language: 'en' | 'de'): {
    suggestedTags: string[];
    confidence: number;
    reasoning: string;
    newTagCandidates: string[];
    ...
    for (const tag of availableTags) {
      ...
      for (const tagWord of tagWords) {
        ...
        if (semanticMatches.some(match => 
          allWords.some(word => word.includes(match))
        )) {
          matches++;
        }
      }
  ...
  private analyzeContentPatterns(
    ...
    if (/I think|I feel|my experience|reflection|personal/i.test(content) || 
    ...
    const technicalTerms = words.filter(word => 
      ...
      ['framework', 'library', 'tool', 'platform', 'technology', 'method'].some(tech => 
        content.toLowerCase().includes(word + ' ' + tech) ||
        content.toLowerCase().includes(tech + ' ' + word)
      )
    );
  ...
  public analyzeFile(filePath: string): ContentAnalysis | null {
    try {
      ...
      const newSuggestedTags = analysis.suggestedTags.filter(tag => 
        !existingTags.map(t => t.toLowerCase()).includes(tag.toLowerCase())
      );
      
      // Determine collection from file path
      const collection = filePath.split(path.sep).find(part => 
        ['books', 'projects', 'lab', 'life', 'pages'].includes(part)
      ) || 'unknown';
