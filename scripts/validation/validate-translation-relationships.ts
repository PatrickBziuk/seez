    ...
    for (const [groupKey, group] of relationships) {
      if (group.length === 1) {
        ...
        for (const translation of translationItems) {
          const originalExists = originalLanguageItems.some(
            (orig) =>
          ...
          const original = originalLanguageItems.find(
            (orig) =>
              orig.canonicalId === translation.translationOf ||
              `${orig.collection}/${orig.slug}` === translation.translationOf
          );
        ...
        for (const item of group) {
          if (item.translationOf && !item.translationOf.startsWith(groupKey)) {
            const referencedExists = allContent.some(
              (content) =>
      ...
      try {
        ...
        for (const item of allContent) {
          const registryEntry = registry.entries?.find(
            (entry: { filePath: string; canonicalId?: string }) => entry.filePath === item.file.replace('./', '')
          );
