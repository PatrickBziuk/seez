...
function calculateMetrics(
  ...
  const usd =
    event.usd ||
    (event.input_tokens * modelConfig.usd_per_1k_input + event.output_tokens * modelConfig.usd_per_1k_output) / 1000;
...
export async function aggregateLedger(): Promise<void> {
  ...
  const sitewideStats: SitewideStats = {
    ...
    updated_at: new Date().toISOString(),
    grid_intensity: gridIntensity,
    sources: ['https://openai.com/pricing', 'https://www.anthropic.com/pricing', 'EEA Greenhouse Gas Emissions'],
  };
  ...
  for (const event of ledgerData) {
    ...
    sitewideStats.operations_by_type[event.op] = (sitewideStats.operations_by_type[event.op] || 0) + 1;
    sitewideStats.models_used[event.model] = (sitewideStats.models_used[event.model] || 0) + 1;
  ...
  console.log(
    `   📊 Sitewide: ${sitewideStats.total_tokens} tokens, $${sitewideStats.total_usd.toFixed(4)}, ${sitewideStats.total_co2_g.toFixed(1)}g CO₂`
  );
