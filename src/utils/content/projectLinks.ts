 ...
 */
export async function getProjectLink(projectName: string, language: 'en' | 'de' = 'en'): Promise<ProjectLink> {
  ...
  try {
    ...
    const project = projects.find((p) => p.id.includes(mapping.slug) && p.data.language === language);
    ...
    const fallbackProject = projects.find((p) => p.id.includes(mapping.slug) && p.data.language === fallbackLang);
 ...
 */
export async function getProjectLinks(projectNames: string[], language: 'en' | 'de' = 'en'): Promise<ProjectLink[]> {
  const links = await Promise.all(projectNames.map((name) => getProjectLink(name, language)));
  return links;
}
 ...
 */
export async function enhanceTextWithProjectLinks(text: string, language: 'en' | 'de' = 'en'): Promise<string> {
