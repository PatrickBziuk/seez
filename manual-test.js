...
function testIsLinkActiveLogic() {
  ...
  testCases.forEach((test) => {
    ...
    console.log(
      `${status} ${test.desc}: href="${test.href}", path="${test.path}", result=${result}, expected=${test.expected}`
    );
  });
}
...
async function testSearchModal() {
  ...
  if (elements.searchModal) {
    const isVisible =
      !elements.searchModal.classList.contains('hidden') && elements.searchModal.style.display !== 'none';
