...
test.describe('Navigation Active State Fix Validation', () => {
  test('should validate that Books link styling is fixed', async ({ page }) => {
    ...
    // On Books page: Books should be active, Projects should not be
    expect(booksPageState.booksActive).toBe(true);
    expect(booksPageState.projectsActive).toBe(false);

    await page.screenshot({
      path: 'test-results/nav-fix-books-page.png',
      fullPage: false,
    });
    ...
    // On Projects page: Projects should be active, Books should NOT be active
    expect(projectsPageState.projectsActive).toBe(true);
    expect(projectsPageState.booksActive).toBe(false);

    await page.screenshot({
      path: 'test-results/nav-fix-projects-page.png',
      fullPage: false,
    });
    ...
    // On Music page: Music should be active, Books should NOT be active
    expect(musicPageState.musicActive).toBe(true);
    expect(musicPageState.booksActive).toBe(false);

    await page.screenshot({
      path: 'test-results/nav-fix-music-page.png',
      fullPage: false,
    });
