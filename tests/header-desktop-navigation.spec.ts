...
test.describe('Header Navigation - Large Desktop Mode', () => {
  ...
  test('should maintain desktop navigation when clicking categories', async ({ page }) => {
    ...
    console.log(`After Books click: Desktop nav visible = ${desktopStillVisible}, Toggle hidden = ${toggleStillHidden}`);
    ...
    if (!desktopStillVisible || !toggleStillHidden) {
      ...
      const desktopNavDisplay = await desktopNavAfter.evaluate(el => 
        window.getComputedStyle(el).display
      );
      const toggleDisplay = await toggleButtonAfter.evaluate(el => 
        window.getComputedStyle(el).display
      );
    ...
    for (const category of categories) {
      ...
      await page.locator(`nav[data-nav="desktop"] a[href="/en/${category}"]`).click();
      await page.waitForLoadState('networkidle');
