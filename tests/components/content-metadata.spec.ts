...
test.describe('Content Metadata Display', () => {
  ...
  test('should display content metadata badges', async ({ page }) => {
    ...
    if (await contentLinks.count() > 0) {
      ...
      const badges = page.locator(
        '.badge, .tag, .metadata-badge, .status-badge, [data-testid="badge"], .chip'
      );
  ...
  test('should show publication/creation timestamps', async ({ page }) => {
    ...
    if (await contentLinks.count() > 0) {
      ...
      const timestamps = page.locator(
        'time, .timestamp, .date, .published, .created, [datetime]'
      );
  ...
  test('should display content tags properly', async ({ page }) => {
    ...
    if (await contentLinks.count() > 0) {
      ...
      const tags = page.locator(
        '.tag, .tags .tag-item, .content-tag, .topic-tag, [data-tag]'
      );
  ...
  test('should link tags to filtered content', async ({ page }) => {
    ...
    if (await contentLinks.count() > 0) {
      ...
      const clickableTags = page.locator(
        'a.tag, .tag a, .tags a, [href*="tag"], [href*="filter"]'
      );
  ...
  test('should be responsive and readable on mobile', async ({ page }) => {
    ...
    if (await contentLinks.count() > 0) {
      ...
      const metadataSection = page.locator(
        '.metadata, .content-metadata, .post-meta, .article-meta'
      );
  ...
  test('should maintain proper spacing and alignment', async ({ page }) => {
    ...
    if (await contentLinks.count() > 0) {
      ...
      const metadataElements = page.locator(
        '.metadata > *, .content-metadata > *, .post-meta > *'
      );
...
test.describe('Metadata Multilingual Support', () => {
  test('should display metadata in current language', async ({ page }) => {
    ...
    await page.goto('/en');
    const enContentLinks = page.locator('a[href*="/en/books/"], a[href*="/en/projects/"], a[href*="/en/lab/"], a[href*="/en/life/"]');
    ...
    if (await enContentLinks.count() > 0) {
      ...
      if (metadataText) {
        ...
        const hasEnglishTerms = englishTerms.some(term => 
    ...
    await page.goto('/de');
    const deContentLinks = page.locator('a[href*="/de/books/"], a[href*="/de/projects/"], a[href*="/de/lab/"], a[href*="/de/life/"]');
  ...
  test('should handle language-specific date formatting', async ({ page }) => {
    ...
    await page.goto('/en');
    let contentLinks = page.locator('a[href*="/en/books/"], a[href*="/en/projects/"], a[href*="/en/lab/"], a[href*="/en/life/"]');
    ...
    await page.goto('/de');
    contentLinks = page.locator('a[href*="/de/books/"], a[href*="/de/projects/"], a[href*="/de/lab/"], a[href*="/de/life/"]');
  ...
  test('should show appropriate language indicators', async ({ page }) => {
    ...
    if (await contentLinks.count() > 0) {
      ...
      const languageIndicators = page.locator(
        '.language-indicator, .lang-badge, [data-lang], .content-language'
      );
      ...
      if (await languageIndicators.count() > 0) {
        ...
        const hasValidLanguage = validLanguages.some(lang => 
          langText?.toLowerCase().includes(lang)
        );
