...
test.describe('TLDR Component Functionality', () => {
  ...
  test('should display TLDR sections', async ({ page }) => {
    ...
    if (await contentLinks.count() > 0) {
      ...
      const tldrElements = page.locator(
        '.tldr, .tl-dr, .summary, .tldr-section, [data-testid="tldr"]'
      );
  ...
  test('should expand and collapse TLDR content', async ({ page }) => {
    ...
    for (let i = 0; i < Math.min(3, await contentLinks.count()); i++) {
      ...
      const tldrToggle = page.locator(
        '.tldr-toggle, .toggle-tldr, .expand-tldr, .tldr button, .tldr [role="button"]'
      );
  ...
  test('should show/hide TLDR content based on state', async ({ page }) => {
    ...
    if (await contentLinks.count() > 0) {
      ...
      const tldrContent = page.locator(
        '.tldr-content, .tldr-text, .tldr-body, .collapsible-content'
      );
      
      const tldrToggle = page.locator(
        '.tldr-toggle, .toggle-tldr, .expand-tldr'
      );
  ...
  test('should have proper ARIA attributes for accessibility', async ({ page }) => {
    ...
    if (await contentLinks.count() > 0) {
      ...
      const tldrToggle = page.locator(
        '.tldr-toggle, .toggle-tldr, .expand-tldr, .tldr button'
      );
      ...
      if (await tldrToggle.count() > 0) {
        ...
        expect(ariaControls || role === 'button' || 
               await tldrToggle.first().evaluate(el => el.tagName === 'BUTTON')).toBeTruthy();
      }
    }
  });
...
test.describe('TLDR Visual Presentation', () => {
  test('should have distinct visual styling', async ({ page }) => {
    ...
    if (await contentLinks.count() > 0) {
      ...
      const tldrSection = page.locator(
        '.tldr, .tl-dr, .summary, .tldr-section'
      );
    ...
    if (await contentLinks.count() > 0) {
      ...
      if (await tldrToggle.count() > 0) {
        ...
        if (toggleText) {
          ...
          const hasExpandText = expandTexts.some(text => 
            toggleText.toLowerCase().includes(text.toLowerCase())
          );
...
test.describe('TLDR Content Quality', () => {
  test('should contain meaningful summary content', async ({ page }) => {
    ...
    if (await contentLinks.count() > 0) {
      ...
      const tldrContent = page.locator(
        '.tldr-content, .tldr-text, .tldr p, .summary-text'
      );
