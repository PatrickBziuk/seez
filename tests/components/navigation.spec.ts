...
test.describe('Main Navigation Menu', () => {
  ...
  test('should have proper semantic structure', async ({ page }) => {
    ...
    if (await navigation.count() > 0) {
      ...
      const isSemanticNav = await navigation.first().evaluate(el => 
  ...
  test('should display breadcrumbs on content pages', async ({ page }) => {
    ...
    if (await contentLinks.count() > 0) {
      ...
      const breadcrumbs = page.locator(
        '.breadcrumb, .breadcrumbs, nav[aria-label*="breadcrumb"], [data-breadcrumb]'
      );
...
test.describe('Footer Navigation', () => {
  ...
  test('should include social media links', async ({ page }) => {
    ...
    const socialLinks = page.locator(
      'footer a[href*="github"], footer a[href*="twitter"], footer a[href*="linkedin"]'
    );
  ...
  test('should display copyright information', async ({ page }) => {
    ...
    const copyright = page.locator(
      'footer .copyright, footer [data-copyright], footer:has-text("©")'
    );
  ...
  test('should adapt navigation for different languages', async ({ page }) => {
    ...
    if (await navLinks.count() > 0) {
      ...
      if (englishTexts.length > 0 && germanTexts.length > 0) {
        const hasDifferentTexts = englishTexts.some((text, index) => 
          germanTexts[index] && text !== germanTexts[index]
        );
