...
test.describe('Search Results Display', () => {
  test('should display search results', async ({ page }) => {
    ...
    if ((await searchInput.count()) > 0) {
      ...
      // Look for results
      const searchResults = page.locator('.search-results, .results, .search-hits, .search-result-item');
  ...
  test('should show result count', async ({ page }) => {
    ...
    if ((await searchInput.count()) > 0) {
      ...
      const resultCount = page.locator('.result-count, .search-count, .results-found, .search-stats');
  ...
  test('should highlight search terms in results', async ({ page }) => {
    ...
    if ((await searchInput.count()) > 0) {
      ...
      const highlights = page.locator('.highlight, .search-highlight, mark, .highlighted-term');
  ...
  test('should display no results message when appropriate', async ({ page }) => {
    ...
    if ((await searchInput.count()) > 0) {
      ...
      const noResults = page.locator('.no-results, .empty-results, .search-empty, [data-no-results]');
...
test.describe('Search Filtering and Collections', () => {
  test('should search across different content collections', async ({ page }) => {
    ...
    if ((await searchInput.count()) > 0) {
      ...
      const mixedResults = page.locator('.result-item[data-collection], .search-result .collection-badge');
  ...
  test('should handle tag-based filtering', async ({ page }) => {
    ...
    if ((await searchInput.count()) > 0) {
      ...
      const tagFilters = page.locator('.tag-filter, .filter-tag, [data-tag-filter]');
...
test.describe('Search Performance and UX', () => {
  test('should show loading state during search', async ({ page }) => {
    ...
    if ((await searchInput.count()) > 0) {
      ...
      const loadingIndicators = page.locator('.loading, .spinner, .search-loading, [data-loading]');
  ...
  test('should implement search suggestions/autocomplete', async ({ page }) => {
    ...
    if ((await searchInput.count()) > 0) {
      ...
      const suggestions = page.locator('.search-suggestions, .autocomplete, .search-dropdown, [data-suggestions]');
  ...
  test('should handle keyboard navigation in suggestions', async ({ page }) => {
    ...
    if ((await searchInput.count()) > 0) {
      ...
      if ((await suggestions.count()) > 0 && (await suggestions.first().isVisible())) {
        ...
        const highlightedSuggestion = page.locator('.suggestion.highlighted, .suggestion:focus, .suggestion.active');
...
test.describe('Search Multilingual Support', () => {
  ...
  test('should have localized search interface', async ({ page }) => {
    ...
    if ((await searchInput.count()) > 0) {
      ...
      if (placeholder) {
        const isGerman = placeholder.toLowerCase().includes('suchen') || placeholder.toLowerCase().includes('finden');
  ...
  test('should handle cross-language search links', async ({ page }) => {
    ...
    if ((await searchInput.count()) > 0) {
      ...
      const languageOptions = page.locator('.search-language-toggle, .lang-filter, [data-search-lang]');
  ...
  test('should have proper ARIA attributes', async ({ page }) => {
    ...
    if ((await searchInput.count()) > 0) {
      ...
      expect(
        ariaLabel ||
          role === 'searchbox' ||
          (await searchInput.first().evaluate((el) => (el as HTMLInputElement).type === 'search'))
      ).toBe(true);
