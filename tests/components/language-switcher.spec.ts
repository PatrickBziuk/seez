...
test.describe('Language Switcher Display', () => {
  ...
  test('should display language switcher component', async ({ page }) => {
    ...
    if (await languageSwitcher.count() > 0) {
      ...
      const languageOptions = languageSwitcher.first().locator(
        '.lang-option, .language-option, option, a, summary, button'
      );
  ...
  test('should show current language as selected', async ({ page }) => {
    ...
    const languageSwitcher = page.locator(
      '.language-switcher, .lang-switcher, .language-toggle'
    );
    
    if (await languageSwitcher.count() > 0) {
      const currentLangElement = languageSwitcher.first().locator(
        '.current-lang, .active, [aria-current="page"], .selected'
      );
  ...
  test('should show available language options', async ({ page }) => {
    ...
    if (await languageSwitcher.count() > 0) {
      ...
      const toggleButton = languageSwitcher.first().locator(
        'button, .dropdown-toggle, .lang-toggle'
      );
      ...
      const languageOptions = page.locator(
        '.lang-option, .language-option, .dropdown-item'
      );
      ...
      if (await languageOptions.count() > 0) {
        ...
        const hasEnglish = allOptionsText.some(text => 
          text.toLowerCase().includes('en') || text.toLowerCase().includes('english')
        );
        const hasGerman = allOptionsText.some(text => 
          text.toLowerCase().includes('de') || text.toLowerCase().includes('deutsch') || text.toLowerCase().includes('german')
        );
...
test.describe('Language Switching Functionality', () => {
  test('should switch between English and German', async ({ page }) => {
    ...
    if (await languageSwitcher.count() > 0) {
      ...
      const germanOption = page.locator(
        'a[href*="/de"].language-option, .lang-option[data-lang="de"]'
      );
  ...
  test('should maintain page context when switching languages', async ({ page }) => {
    ...
    if (await contentLinks.count() > 0) {
      ...
      const germanOption = page.locator(
        'a[href*="/de"], .lang-option[data-lang="de"]'
      );
    ...
    if (await contentLinks.count() > 0) {
      ...
      if (await germanOption.count() > 0) {
        ...
        const errorIndicators = page.locator(
          '.error, .not-found, h1:has-text("404"), h1:has-text("Error")'
        );
  ...
  test('should update page metadata when switching languages', async ({ page }) => {
    ...
    const languageSwitcher = page.locator(
      '.language-switcher-wrapper, .language-switcher, .lang-switcher'
    );
...
test.describe('Language Switcher Dropdown Behavior', () => {
  test('should open and close dropdown properly', async ({ page }) => {
    ...
    const dropdownToggle = page.locator(
      '.language-switcher button, .lang-switcher button, .dropdown-toggle'
    );
    
    if (await dropdownToggle.count() > 0) {
      // Initially, dropdown should be closed
      const dropdownMenu = page.locator(
        '.dropdown-menu, .lang-options, .language-options'
      );
  ...
  test('should close dropdown when clicking outside', async ({ page }) => {
    ...
    const dropdownToggle = page.locator(
      '.language-switcher button, .lang-switcher button'
    );
...
test.describe('Language Switcher Visual Design', () => {
  test('should have proper styling and indicators', async ({ page }) => {
    ...
    if (await languageSwitcher.count() > 0) {
      ...
      expect(styles.cursor === 'pointer' || 
             styles.border !== '0px none' || 
             styles.backgroundColor !== 'rgba(0, 0, 0, 0)').toBe(true);
    }
  });
  ...
  test('should display language flags or codes', async ({ page }) => {
    ...
    if (await languageSwitcher.count() > 0) {
      ...
      expect(hasFlags || hasLangCodes || 
             switcherText?.match(/EN|DE|English|Deutsch/i)).toBeTruthy();
    }
  });
  ...
  test('should work with dark mode', async ({ page }) => {
    ...
    const darkModeToggle = page.locator(
      '.dark-mode-toggle, .theme-toggle, [data-theme-toggle]'
    );
...
test.describe('Language Switcher Accessibility', () => {
  ...
  test('should have accessible language labels', async ({ page }) => {
    ...
    const languageOptions = page.locator(
      '.lang-option, .language-option, [data-lang]'
    );
  ...
  test('should have sufficient color contrast', async ({ page }) => {
    ...
    if (await languageSwitcher.count() > 0) {
      ...
      expect(colorInfo.backgroundColor !== 'rgba(0, 0, 0, 0)' || 
             colorInfo.border !== '0px none').toBe(true);
    }
  });
});
