  ...
  test('should persist theme preference in localStorage', async ({ page }) => {
    ...
    if ((await themeToggle.count()) > 0) {
      ...
      const storedTheme = await page.evaluate(() => {
        return (
          localStorage.getItem('theme') || localStorage.getItem('darkMode') || localStorage.getItem('color-scheme')
        );
      });
...
test.describe('Theme System Integration', () => {
  ...
  test('should have proper ARIA attributes for theme toggle', async ({ page }) => {
    ...
    if ((await themeToggle.count()) > 0) {
      ...
      expect(
        ariaLabel || role === 'button' || (await themeToggle.first().evaluate((el) => el.tagName === 'BUTTON'))
      ).toBeTruthy();
    }
  });
