...
test.describe('Content Metadata Display', () => {
  test('should display publication date', async ({ page }) => {
    ...
    if (await contentLinks.count() > 0) {
      ...
      const dateElements = page.locator(
        '.date, .published, .timestamp, time, [datetime], .publication-date'
      );
  ...
  test('should display author information', async ({ page }) => {
    ...
    if (await contentLinks.count() > 0) {
      ...
      const authorElements = page.locator(
        '.author, .by-author, .written-by, [rel="author"]'
      );
  ...
  test('should display tags or categories', async ({ page }) => {
    ...
    if (await contentLinks.count() > 0) {
      ...
      const tagElements = page.locator(
        '.tags, .categories, .tag, .category, .label, .badge'
      );
  ...
  test('should display reading time or word count', async ({ page }) => {
    ...
    if (await contentLinks.count() > 0) {
      ...
      const readingTimeElements = page.locator(
        '.reading-time, .read-time, .duration, .word-count, .time-to-read'
      );
...
test.describe('Multilingual Content Rendering', () => {
  ...
  test('should handle translation links properly', async ({ page }) => {
    ...
    const translationLinks = page.locator(
      '.translation-links, .language-versions, a[href*="/de/"], a[href*="/en/"]'
    );
